...
# 入出力ファイル名
input_file = "patch-qe-7.4.1"
output_file = "patch-qe-7.4.1.spack"

# 正規表現パターン
diff_pattern = re.compile(r'^(diff -ru )a/qe-7\.4\.1/(.+?) b/qe-7\.4\.1/(.+)$')
file_pattern = re.compile(r'^(---|\+\+\+) (a|b)/qe-7\.4\.1/(.+)$')
