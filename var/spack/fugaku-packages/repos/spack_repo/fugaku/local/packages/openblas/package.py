...
class Openblas(CMakePackage, MakefilePackage):
    ...
    patch("openblas-0.3.33-fj-link.patch", when="@0.3.33 %fj")
...
class MakefileBuilder(makefile.MakefileBuilder):
    ...
    def make_defs(self):
        ...
        if self.spec.satisfies("%fj"):
            fj_ldflags = "-lfj90i -lfj90f -lfjsrcinfo -lelf"
            if self.spec.satisfies("@0.3.33 threads=openmp"):
                # libfj90i also exports omp_*; select the OpenMP runtime first.
                fj_ldflags = "-lfjomphk -lfjomp " + fj_ldflags
            make_defs.append("LDFLAGS=" + fj_ldflags)
