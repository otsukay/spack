...
class QuantumEspresso(CMakePackage, Package):
    ...
    build_system(conditional("cmake", when="@6.8:"), "generic", default="generic")
    ...
    # Keep provider selection in the Spack environment. These constraints
    # only apply if a Fujitsu implementation was selected for a virtual.
    requires(
        "^fujitsu-fftw+openmp",
        when="+openmp ^[virtuals=fftw-api] fujitsu-fftw",
    )
    requires(
        "^fujitsu-fftw+mpi precision=double",
        when="+mpi ^[virtuals=fftw-api] fujitsu-fftw",
    )
    requires(
        "^fujitsu-ssl2+parallel",
        when="+openmp ^[virtuals=blas] fujitsu-ssl2",
    )
    ...
    # The old FoX layout patched here is no longer present in QE 7.4.1.
    patch("fj-fox.patch", when="@:6.7+patch %fj")

    # Port the original Fugaku fixes to the builtin 7.4.1 source archive.
    patch("fugaku-7.4.1.patch", when="@7.4.1 target=a64fx %fj")
    ...
    def setup_build_environment(self, env):
        if self.spec.satisfies("@7.4.1 target=a64fx build_system=generic %fj"):
...
class GenericBuilder(generic.GenericBuilder):
    def install(self, pkg, spec, prefix):
        ...
        if "+mpi" in spec:
            ...
            options.append("FC={0}".format(mpi.mpifc))
        ...
        if spec.satisfies("@7.4.1 target=a64fx %fj"):
            # Preserve the original Fugaku optimization and BEEF settings.
            # Leave feature definitions to configure so variants keep working.
            optflags = "-Kfast,parallel,openmp,SVE" if "+openmp" in spec else "-Kfast,SVE"
            optflags += " -Knofp_relaxed"
            cppflags = "-traditional -Duse_beef"
            if "+openmp" in spec:
                cppflags += " -D_OPENMP"
            options.extend(
                [
                    "--host=aarch64-linux-gnu",
                    "CFLAGS=-Nnoclang {0} -Duse_beef".format(optflags),
                    "CPPFLAGS={0}".format(cppflags),
                    "FFLAGS={0} -Nalloc_assign -Nlst=t -Free".format(optflags),
                    "LDFLAGS={0}".format(optflags),
                ]
            )
        ...
        if "^[virtuals=fftw-api] fujitsu-fftw" in spec:
            fftw = spec["fftw-api"]
            options.append("FFTW_INCLUDE={0}".format(fftw.prefix.include))
            interfaces = []
            if "+mpi" in spec:
                interfaces.append("mpi")
            if "+openmp" in spec:
                interfaces.append("openmp")
            query = "fftw-api" + (":" + ",".join(interfaces) if interfaces else "")
            options.append("FFT_LIBS={0}".format(spec[query].libs.ld_flags))
        ...
        if spec.satisfies("@:6.7") or spec.satisfies("@7.4.1 target=a64fx %fj"):
